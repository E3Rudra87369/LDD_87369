...
static void fifo_delete(struct timer_list *tm) {
    int ret;
    if(kfifo_is_empty(&mybuf)){
        del_timer(&timer);
        pr_info("%s: timer is destroyed.\n", THIS_MODULE->name);
        flag = 0;
        return;
    }
    ret=kfifo_out(&mybuf, buf, 1);
    pr_info("%s:data from kfifo %s no. of bytes %d\n", THIS_MODULE->name, buf,ret);
...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param) {
    ...
    case FIFO_GETINFO:
       ...
       case FIFO_START_TIMER:
        timer_setup(&timer, fifo_delete, 0);
       ...
       if(flag==1){
        ...
        pr_info("%s: timer has not been setup yet.\n", THIS_MODULE->name);
       }
       return 0;
